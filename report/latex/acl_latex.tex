...
\author{Ismail Elyamany \And Huang Chongtian \And Xia Yitong \And Zhu Ruijie \\
  ...
  \texttt{\{ismail001, chongtia002, xiay0005, zhur0016\}@e.ntu.edu.sg}}
...
sweep, and organise our contributions into seven buckets: classical loss and
prompt ablations, advanced regularisation, a \emph{preprocessing axis}
...
model soup, and an exhaustive prediction-space ensemble search. Our best
single model is definition prompting at relaxed F1 $= 0.613$. Our best
overall system is a four-model majority-vote ensemble at relaxed F1 $=
0.635$, beating the best published baseline on the same dev split by
$+0.025$. The main lesson is that training-objective diversity, combined
...
published baseline on this version of the dataset reaches relaxed F1
$= 0.610$ and human annotator agreement is around $0.81$
\citep{garg2025reddit}, leaving a $+0.20$ headroom that no single
well-known recipe closes.
...
run in both a raw-text regime and a mojibake/URL/emoji-cleaned regime, so
preprocessing is a first-class experimental variable rather than a
confound (\S\ref{sec:preproc}).
\item Seven contribution buckets on top of DeBERTa: focal loss,
definition prompting, multi-task learning, recall-boost O-downweighting,
R-Drop, FGM~$+$~SWA, structured prediction pipelines, weight-space model
soup, and prediction-space ensemble search (\S\ref{sec:methods}).
...
Reddit-specific pre-training \citep{ji2020socbert} and mental-health-domain
...
opioid-related subreddits, pre-split into train, dev, and test. The
training and dev splits are both Clinical-heavy, while the test split
flips to roughly 70\% Social. Figure~\ref{fig:eda} visualises the split
...
\paragraph{Domain-adapted encoders.} SocBERT \citep{ji2020socbert} is
...
toward the under-represented Social class.
...
\paragraph{R-Drop.} We run each batch through the encoder twice with two
independent dropout masks and add a symmetric KL penalty between the two
...
SocBERT (Reddit-domain) and StressRoBERTa (mental-health) are swapped in
under the transformer token-classifier template while keeping the same
optimiser, LR sweep, and training schedule.

\subsubsection{Bucket 6: Weight-space model soup}
For a given family we fine-tune at every $(\mathrm{lr},\,\mathrm{seed})$
configuration and average the weights of the top-$K$ dev-F1 checkpoints:
...
\paragraph{Setup.} All experiments use AdamW with a linear warmup of
10\% of total steps and linear decay. DeBERTa family trains with batch
size 16 for 12 epochs, dropout $0.1$, max sequence length 512, and
gradient clipping at $1.0$. BiLSTM--CRF trains for 15 epochs with SGD
$0.01$. GLiNER fine-tunes with the authors' default schedule.
...
pattern is striking: \emph{only} definition prompting improves on
the baseline in either regime. Focal loss, multi-task, recall-boost,
R-Drop, and FGM~$+$~SWA all sit below their own-regime baseline when
...
Hierarchy drifts up after epoch~6, signalling mild overfit on this
small training set.}
\label{fig:curves}
\end{figure}
...
over the best single model comes from.
...
variants is not exposed by the toolkit.}
\label{tab:full}
\end{table}
...
\paragraph{Domain adaptation is not a free lunch.} SocBERT and
...
task, organised as seven contribution buckets layered on top of a
DeBERTa backbone under a consistent LR sweep and a paired
raw-vs-preprocessed protocol. Our best single model, definition
...
baseline by $+0.025$ on the same dev split. The pattern across buckets
is consistent: no single regularisation trick beats a well-tuned
DeBERTa, and the headroom comes from combining decorrelated recipes
via hard voting. We see the $\approx 0.18$ gap to the human ceiling
as coming mainly from boundary decoding and implicit-expression
recall, and view these as the most promising future-work directions.
...
after epoch $10$. BiLSTM--CRF trains for $15$ epochs with SGD
$0.01$. GLiNER fine-tuning follows the default schedule of
