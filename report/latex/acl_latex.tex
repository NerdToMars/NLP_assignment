...
$= 0.610$, which uses a pretrained DeBERTa-large \citep{he2021deberta} model.
...
ge2024reddit}. The dominant approach is a transformer encoder with a
token-classification head, sometimes augmented with domain adaptation.
Reddit-Impacts~2.0 \citep{ge2024reddit} is unusual in separating \emph{clinical} and \emph{social} outcomes and in deliberately flipping the label prior between train and test.

\paragraph{Domain-adapted encoders.}
Reddit-specific pre-training \citep{guo2023socbert} and mental-health-domain pre-training \citep{alqahtani2026stressroberta} typically help when the downstream task shares vocabulary and register with the pre-training corpus. In our setting, however, the task labels are not register-bound and generic
...
\citep{wu2021rdrop}, FGM adversarial training
...
flips to roughly 70\% Social. Figure~\ref{fig:eda} visualizes the split
...
pre-trained on Reddit posts; StressRoBERTa \citep{alqahtani2026stressroberta} is
...
\item[Sentence~$+$~Token Hierarchy.] A single model with two heads trained jointly: one for sentence presence, one for token BIO.
...
The Two-Step Impact Pipeline trades recall for precision and achieves the highest strict F1 among single-model recipes ($0.404$).
...
\bottomrule
\end{tabular}
\caption{Full LR-selected ablation (dev F1). Strict F1 for GLiNER variants is not exposed by the toolkit.}
\label{tab:full}
\end{table}
...
\paragraph{Domain adaptation is not a free lunch.} SocBERT and StressRoBERTa, both plausible a priori, underperform general-purpose DeBERTa by $\approx 0.09$--$0.19$ relaxed F1. For a task whose labels are semantic categories defined at annotation time, generic language understanding appears to matter more than register-specific pre-training.
