...
\begin{frame}{Innovation 1: Distribution-Aware Focal Loss}
  Focal loss with $\gamma=2.0$. Social class weight $\times 1.5$.
  \medskip
  \begin{center}\small
  \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    DeBERTa baseline & \textbf{0.566} & \textbf{0.400} \\
  ...
  Social tokens. Both relaxed and strict F1 drop sharply by $-0.038$ and $-0.110$.
...
\begin{frame}{Innovation 2: Definition Prompting}
  ...
  \begin{tabular}{lcc}
    ...
    + Definition prompting & \textbf{0.572} & \textbf{0.402} \\
    \bottomrule
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Innovation 3: Multi-task Auxiliary Head}
  ...
  \textbf{Result.} Relaxed F1 $= 0.529$ ($-0.037$ compared to baseline $0.566$).
  \smallskip
...
\begin{frame}{Training Dynamics Across Recipes}
  ...
  in training -- regularization is effective, even when the F1 curve has flattened.
\end{frame}

% ---------- 12. Ensemble ----------
\begin{frame}{Innovation 7: Ensemble Search -- Majority Vote Wins}
  Comprehensive search over subsets of the top 9 LR-selected models,
...
\begin{frame}{Three Key Insights}
  \begin{enumerate}
    ...
    \item \textbf{Majority vote beats probability averaging.}
          ...
          than soft averaging ($0.602$). Confident-but-occasionally-wrong models don't combine well via logits.
  \end{enumerate}
\end{frame}
